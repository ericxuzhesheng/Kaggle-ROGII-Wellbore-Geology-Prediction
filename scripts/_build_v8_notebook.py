"""Build kaggle_kernel_v8_robust.ipynb — log-driven robustness fixes over V7.

Based on V7 run log analysis (OOF=10.6173, Fold 0=12.03, Fold 3 underfit):
- Change 1: robust full-retrain iters (median + p75 lower bound) — fix underfit
- Change 2: NaN/Inf sanitize in build_well — fix Fold 3 early-stop anomaly
- Change 3: alpha grid 0.70–1.15, tau grid extended — pp hit upper bound in V7
- Change 4: Stratified GroupKFold by well length — balance Fold 0 difficulty
- Change 5: LGB seeds diversified (num_leaves/lr/min_child_samples)
- Change 6: LGB early_stopping_rounds 200 → 300
"""
import json
...
# ===========================================================================
CELL_HEADER = """\
# v8 Robust — log-driven fixes over v7

V7 OOF=10.6173 was inflated by Fold 0 (RMSE=12.03) and Fold 3 (severe early-stop).
v8 addresses both with the following targeted changes:

| Change | Detail |
|--------|--------|
| **Robust retrain iter** | `final_n_iters` uses `max(median, p75 × 0.8) × 1.10` — Fold 3's 74/79 no longer drag mean down |
| **NaN/Inf sanitize** | `build_well` cleans Inf and column-median fills NaN before return — likely root cause of Fold 3 anomaly |
| **Wider pp grid** | alpha 0.70–1.15 (V7 capped at 1.00), tau adds 75/150 |
| **Stratified split** | Group folds by well-length quintile so Fold 0 no longer concentrates hardest wells |
| **Diversified LGB** | seeds 42/7/123 now also differ in `num_leaves` (127/95/63) and `learning_rate` (0.035/0.040/0.030) |
| **LGB patience 300** | early stopping more tolerant (was 200) |

Preserved from V7: per-fold KNN rebuild, IDW fallback, per-fold pp + median, DTW features, full retrain, Nelder-Mead.
"""
...
def build_well(hw_path, tw_path, is_train):
    ...
    _SAFE_ZERO_PREFIX = ("tda", "tdbc", "tdsc", "tdpf", "dtw_diff_",
...
# V7 had Fold 0 RMSE=12.0 while others ~10 — likely concentrated the longest/hardest wells.
# Sort by length, then assign chunks of N_SPLITS wells permuted across folds so each fold
# sees a balanced length distribution.
_well_lens = {}
for w in train_wids:
    try:
...
CELL_RETRAIN = """\
# v8: robust iter estimation — median + p75 lower bound so anomalous Fold 3 (iter=74/79)
# no longer drags down the mean. Reduces underfit risk in full retrain.
...
out = ROOT / "kaggle_kernel_v8_robust.ipynb"
